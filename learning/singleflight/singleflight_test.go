...
func TestDoDuplicate(t *testing.T) {
	fn := func() (interface{}, error) {
		fmt.Println("func invoked")
		time.Sleep(100 * time.Millisecond) // let more goroutines enter Do

		return 1, nil
	}
	const n = 10

	fmt.Println("----------------func invoke N times----------------")
	for i := 0; i < n; i++ {
		go func() {
			v, _ := fn()
			fmt.Printf("----------------func invoke res: val: %d----------------\n", v)
		}()
	}

	time.Sleep(time.Second)

	fmt.Println("----------------func invoke 1 times----------------")
	var g Group
	for i := 0; i < n; i++ {
		go func() {
			v, _, _ := g.Do("ryoma", fn)
